...
Write-Host "🔧 设置 Cloudflare Workers 环境变量..." -ForegroundColor Green
...
foreach ($key in $envVars.Keys) {
    $value = $envVars[$key]
    Write-Host "设置 $key = $value" -ForegroundColor Yellow
    wrangler secret put $key --name kemono-shares-api --preview
}

Write-Host "✅ 环境变量设置完成！" -ForegroundColor Green
Write-Host "📝 请检查 Cloudflare Workers 控制台确认变量已设置" -ForegroundColor Cyan 